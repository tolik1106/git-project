...
public class Solution {
    ...
    public Set<Integer> getAllFriendsAndPotentialFriends(int index, int deep) {
        ...
        set = getFriengs(humansRelationships, index);
        for (int i = 0; i < deep - 1; i++) {
            set.addAll(getFriendsAndPotentialFriends(humansRelationships, set));
        }

        set.removeIf(i -> i == index);
        return set;
    }
    ...
    private static Set<Integer> getFriengs(boolean[][] list, int index) {
        Set<Integer> set = new TreeSet<>();

        for (int i = 0; i < list.length; i++) {
            if ((i < index) && (index < list.length) && list[index][i]) {
                set.add(i);
            } else if ((i > index) && list[i][index]) {
                set.add(i);
            }
        }
        return set;
    ...
    private static Set<Integer> getFriendsAndPotentialFriends(boolean[][] list, Set<Integer> people) {
        Set<Integer> set = new TreeSet<>();
        for (Integer i: people) {
            set.addAll(getFriengs(list, i));
        }
        return  set;
    }
    //return test data
